class ModelSetSolr < ModelSet
  MAX_SOLR_RESULTS = 1000

  def add_joins!(*conditions)
    raise 'joins not supported in solr'
  end
  ...
  def aggregate(query)
    raise 'aggregate queries not supported by ModelSetSolr'
  end

  def limited_aggregate(query)
    aggregate(query)
  end
